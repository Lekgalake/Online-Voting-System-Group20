...
-- Participation eligibility (before insert)
CREATE OR REPLACE FUNCTION enforce_participation_eligibility()
RETURNS TRIGGER AS $$
DECLARE elig RECORD;
BEGIN
    SELECT * INTO elig FROM check_voter_eligibility(NEW.voter_id, NEW.election_id) LIMIT 1;
    IF NOT elig.is_eligible THEN RAISE EXCEPTION '%', elig.reason; END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_participation_eligibility
BEFORE INSERT ON participation FOR EACH ROW
EXECUTE FUNCTION enforce_participation_eligibility();

-- Atomic dual-insert vote
CREATE OR REPLACE FUNCTION cast_vote(
    p_voter_id CHAR(13), p_election_id INTEGER, p_candidate_id INTEGER
) RETURNS INTEGER AS $$
DECLARE v_receipt_id INTEGER; elig RECORD;
BEGIN
    IF NOT EXISTS (SELECT 1 FROM candidate WHERE candidate_id = p_candidate_id AND election_id = p_election_id) THEN
        RAISE EXCEPTION 'Candidate does not belong to this election';
    END IF;
    SELECT * INTO elig FROM check_voter_eligibility(p_voter_id, p_election_id) LIMIT 1;
    IF NOT elig.is_eligible THEN RAISE EXCEPTION '%', elig.reason; END IF;
    INSERT INTO participation (voter_id, election_id) VALUES (p_voter_id, p_election_id);
    INSERT INTO anonymous_vote (candidate_id, election_id) VALUES (p_candidate_id, p_election_id)
        RETURNING vote_receipt_id INTO v_receipt_id;
    INSERT INTO audit_log (voter_id, action, log_time)
        VALUES (p_voter_id, 'Vote cast in election ID: ' || p_election_id, CURRENT_TIMESTAMP);
    RETURN v_receipt_id;
END;
...
BEGIN RAISE EXCEPTION 'Audit logs are immutable'; END; $$ LANGUAGE plpgsql;

CREATE TRIGGER trg_audit_log_no_update BEFORE UPDATE ON audit_log FOR EACH ROW EXECUTE FUNCTION prevent_audit_log_mutation();
CREATE TRIGGER trg_audit_log_no_delete BEFORE DELETE ON audit_log FOR EACH ROW EXECUTE FUNCTION prevent_audit_log_mutation();

-- Voter ID validation on register
CREATE OR REPLACE FUNCTION enforce_voter_id_valid() RETURNS TRIGGER AS $$
DECLARE parsed RECORD;
BEGIN
    SELECT * INTO parsed FROM parse_sa_id(NEW.voter_id) LIMIT 1;
    IF NOT parsed.valid THEN RAISE EXCEPTION '%', parsed.error_message; END IF;
    IF parsed.age < 18 THEN RAISE EXCEPTION 'Voter must be at least 18 years old'; END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_voter_id_valid
BEFORE INSERT OR UPDATE OF voter_id ON voter FOR EACH ROW EXECUTE FUNCTION enforce_voter_id_valid();

-- Election must have candidates before Active; unlock results when Closed
CREATE OR REPLACE FUNCTION manage_election_status_transitions() RETURNS TRIGGER AS $$
BEGIN
    IF NEW.status = 'Active' AND OLD.status IS DISTINCT FROM 'Active' THEN
        IF NOT EXISTS (SELECT 1 FROM candidate WHERE election_id = NEW.election_id) THEN
...
BEFORE UPDATE OF status, result_locked ON election FOR EACH ROW
